...
class Datetime(arrow.Arrow):
    ...
    def __init__(self, *argv):
        ...
        <Datetime [2017-02-05T08:26:58.861782+00:00]>
        >>> Datetime(2017)  # 可以省略月、日等值
        <Datetime [2017-01-01T00:00:00+00:00]>

        >>> Datetime('2019年3月6日', '%Y年%m月%d日')  # 指定格式
        <Datetime [2019-03-06T00:00:00+00:00]>
        >>> Datetime('w200301周日', 'w%y%m%d周日')  # 周日必须写全，有缺失会报ValueError
        <Datetime [2020-03-01T00:00:00+00:00]>

        >>> Datetime(180213)
        <Datetime [2018-02-13T00:00:00+00:00]>
        >>> Datetime('180213')
        <Datetime [2018-02-13T00:00:00+00:00]>

        >>> Datetime('2015-06-15_22-19-01_HDR.jpg')
        <Datetime [2015-06-15T22:19:01+00:00]>
        >>> Datetime('IMG_20150615_2219011234_HDR.jpg')
        <Datetime [2015-06-15T22:19:01.001234+00:00]>
        >>> Datetime('_2015.6.15_22:19:02')
        <Datetime [2015-06-15T22:19:02+00:00]>

        # 获取文件的创建时间，st_ctime获得的事timestamp格式 1579529472.2958975
        >> Datetime(os.stat(r"C:\pycode\code4101py\ckz.py").st_ctime)
        <Datetime [2020-01-20T22:11:12.295897+00:00]>
        """
        dt = None
        # 1、没有参数则默认当前运行时间
        if not argv:
