...
def _读取WPS问卷提醒数据(kqbook):
    """读取问卷提醒所需字段，避开 WPS 三列 sqlSelect 的 30 秒超时。"""
    df_course = kqbook.sql_select('问卷星', ['序号', '1、所属课程'], 4)
    df_status = kqbook.sql_select('问卷星', ['序号', '处理状态'], 4)

    if len(df_course) == 0:
        return pd.DataFrame(columns=['序号', '1、所属课程', '处理状态'])

    df_course = df_course.drop_duplicates(subset=['序号'], keep='last')
    df_status = df_status.drop_duplicates(subset=['序号'], keep='last')
    df = df_course.merge(df_status, on='序号', how='left')
    _fill_text_na(df, '')
    return df


def _读取CodeYun问卷提醒数据(api_url=CodeYun问卷数据接口, page_size=CodeYun问卷数据页大小):
    """从 CodeYun 公网接口分页读取未处理问卷的摘要数据。"""
    rows = []
    page = 1
    total = None
...
def _读取问卷提醒数据():
    """优先从 CodeYun 读取；失败时回退旧版 WPS 问卷表。"""
    try:
        return _读取CodeYun问卷提醒数据()
    except Exception as e:
        logger.warning(f'从 CodeYun 读取问卷提醒数据失败，回退 WPS：{e}')

    from .db import KqBook

    kqbook = KqBook()
    return _读取WPS问卷提醒数据(kqbook)
