...
"""
这是一套基于labelme标注来进行桌面自动化操作的库，Anto GUI，简称An
...
class ImageTools:
    ...
    def _find_with_pyautogui(cls, img, haystack, params: LocateParams):
        ...
        try:
            boxes = pyautogui.locateAll(img, haystack, grayscale=params.grayscale, confidence=params.confidence)
...
class _AnShapePupil(_AnShapeBasic):
    ...
    def click(self, target=None, x_bias=None, y_bias=None, **kwargs):
        ...
        if params.wait_change and before_click_shot is not None:
            ...
            xlwait(check_changed, timeout=wait_params.timeout, interval=wait_params.interval)
...
class AnShape(_AnShapePupil):
    ...
    def wait_img_notchange(self, **resolved_params):
        ...
        return xlwait(check_not_change, timeout=wait_params.timeout, interval=interval)
...
class AnView(AnShape):
    ...
    def add_inner_shapes(self):
        ...
        for sp in parent_view.shapes:
            ...
            if (
                sp_ltrb[0] >= self_ltrb[0]
                and sp_ltrb[1] >= self_ltrb[1]
                and sp_ltrb[2] <= self_ltrb[2]
                and sp_ltrb[3] <= self_ltrb[3]
            ):
                new_sp = AnShape({"text": sp["text"]}, parent=self)
                new_sp["xywh"] = [
                    sp["xywh"][0] - dx - self["xywh"][0],
    ...
    def save_labelme_file(self, impath=None, save_image=True):
        ...
        for sp in self.shapes:
            ...
            shape = LabelmeDict.gen_shape(
                json.dumps(sp.data, ensure_ascii=False, default=str),
                sp["points"],
                sp["shape_type"],
